...
description: Use when implementing any feature or bugfix, before writing implementation code - write the test first, watch it fail, then write minimal code to pass; enforces web search discipline at the RED phase entry, the GREEN phase entry, and the refactor decision, so each TDD step reflects current 2026 best practice (test framework conventions, mocking guidance, refactor scope) rather than stale training data
---
